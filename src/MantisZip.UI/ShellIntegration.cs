...
internal static class ShellIntegration
{
    ...
    /// <summary>
    /// L.T(L.Shell_Compress)格式扩展名逐个关联独立 ProgId。
    /// 写入 HKCU\Software\Classes，无需管理员权限。
    /// </summary>
    ...
    /// <summary>
    /// L.T(L.Settings_Assoc_Uninstall)：删除所有独立 ProgId 和扩展名的 OpenWithProgids 条目。
    /// </summary>
    public static void UninstallAssociations()
    {
        ...
        try
        {
            foreach (var ext in AppSettings.Instance.CustomAssocExtensions)
            {
                UninstallAssociationForExtension(ext);
            }
        }
        ...
        // 清理旧版单 ProgId（v0.5 以前）
        DeleteRegistryKey($@"Software\Classes\{OldProgId}");

        App.LogDebug("ShellIntegration.UninstallAssociations: done");
    }

    /// <summary>返回已安装文件关联的扩展名数量（仅统计 UI 中可见的扩展名）。</summary>
