...
| ✅ | 设置窗口（6 标签页：压缩 / 解压 / 菜单 / 预览 / 高级 / 关于） | v0.1.3 |
...
| ☐ | 暗色主题 → [详情](docs/PLAN.md#近期p2) | |
| ☐ | 国际化（中 / 英）→ [详情](docs/PLAN.md#近期p2) | |

### 系统集成 (System Integration)

| 状态 | 功能 | 详情 |
|:----:|------|------|
| ✅ | Shell 右键菜单 — 动词模式 / 层叠子菜单双模式，per-verb 开关，AppliesTo 过滤器 | v0.1.3 |
| ✅ | 文件关联（打开方式） | v0.1.3 |
| ✅ | CLI 入口点（`--compress` / `--extract` / `--open` / `--install-shell` / `--uninstall-shell`） | v0.1.3 |
...
| ☐ | 右键菜单目录结构预览 → [详情](docs/PLAN.md#远期p3) | |
| ☐ | 外部工具视频元数据（ffprobe 时长 / 分辨率）→ [详情](docs/PLAN.md#远期p3) | |
