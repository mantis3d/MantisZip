...
- 7z **压缩**使用 [SharpSevenZip](https://github.com/sevenzipsharp/SevenZipSharp)（7z.dll 原生绑定），不再依赖外部 7z.exe。
...
| ✅ | 7z 压缩/解压 | v0.1.0 / v0.1.1，现使用 SharpSevenZip（7z.dll） |
...
| ✅ | 添加到压缩包 | v0.2.9 |
| ✅ | 从压缩包删除 | v0.2.9 |
| ✅ | 压缩包注释（编辑已有 + 压缩时指定） | v0.3.1 |
| ✅ | 注释分配策略（AllSame/FirstOnly/PerLine） | v0.3.1 |
| ✅ | **引擎统一迁移** → [详细设计](.sisyphus/plans/engine-unification-sharpcompress.md) | SharpZipLib → SharpCompress，7z.exe → SharpSevenZip ✅ |
...
| ✅ | Markdown / HTML 渲染（Markdig + WebView2） PDF 内容渲染 | v0.1.5 / v0.3.1 |
...
| ✅ | 压缩配置 TabControl（通用 + 注释双标签，注释分配策略） | v0.3.1 |
...
- [SharpZipLib](https://github.com/icsharpcode/SharpZipLib) — 遗留 ZIP/TAR/GZ 引擎，逐步迁移中（MIT）
