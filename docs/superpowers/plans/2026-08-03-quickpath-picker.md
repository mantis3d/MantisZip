...
- [x] 写失败测试：`QuickPathPickerDirectoryNormalizationTests.cs` 测 `QuickPathPicker.CoerceToDirectory(string)`：
  - 目录路径 `C:\foo` → 原样
  ...
  - null/空 → 透传（或空串，按现定）
- [x] 运行测试确认失败
- [x] 实现静态方法 `internal static string CoerceToDirectory(string picked)`（`Directory.Exists`→原样；`File.Exists`→`Path.GetDirectoryName`；null 安全；否则原样）
- [x] 运行测试通过
- [x] commit: `feat(QuickPathPicker): add directory coercion helper`（并入 726ce62）

### 任务 2 — 本地化 key

- [x] `strings.zh-CN.json` 加 `"QuickPath_Browse": "📁 浏览…"`
- [x] `strings.en.json` 加 `"QuickPath_Browse": "📁 Browse…"`
- [x] lsp 无错；不提交（随控件一起）

### 任务 3 — 控件 XAML + code-behind 骨架

- [x] `QuickPathPicker.axaml`：两行 Grid
...
- [x] `QuickPathPicker.axaml.cs`：
  - `Path` StyledProperty（TwoWay，`OnPathChanged` → 同步 `PathInput.Text` 与 `SyncQuickPathControl`/`SetCurrentPath`）
...
- [x] `lsp_diagnostics` 干净
- [x] build `dotnet build src\...\MantisZip.UI.Avalonia.csproj`
- [x] **commit** `feat(QuickPathPicker): add self-contained reusable path picker`（726ce62）

（此任务较大；推荐拆子步骤逐个验证再 commit，见下方细化）

### 任务 4 — SettingsWindow 接入（默认浏览 = 零配置）

- [x] `settings.axaml` 找到「默认路径优先级」手动路径 TextBox+旧📁，替换为 `<controls:QuickPathPicker x:Name="CustomPathPicker" Path="{Binding CustomPath}" />`
- [x] 移除旧 single-panel + 关联 handled code（若存在于 SettingsWindow.axaml.cs）
- [x] lsp 干净 + `dotnet build`
- [x] **commit** `refactor(settings): use QuickPathPicker for custom default path`（3096693，5 insert / 63 delete）

### 任务 5 — CompressSettingsWindow 接入（注入 SaveFile 浏览）

- [x] Code-behind 里把现有 `OutputFav/Hist/Win` Popup+`AddHandler`+三个 QuickPathControl 拆掉
- [x] 加一个 `QuickPathPicker` 实例（XAML `<controls:QuickPathPicker x:Name="OutputPathPicker" Path="{Binding OutputDirectory}" />`），`BrowseAction = (owner, cur) =>` 回调原 SaveFile（格式联动）逻辑，拆目录/文件名后返回目录
- [x] 移除 `QuickFavButton_Click/QuickBrowseButton_Click/CloseOutputPopups/OutputPathControl_PathSelected/SyncOutputPathControl` 等被替代 handler
- [x] lsp + build
- [x] commit `refactor(compress): use QuickPathPicker for output dir`（并入 cd6778f）

### 任务 6 — ExtractSettingsWindow 接入（注入 ExtractFolder 浏览）

- [x] 移除现有 `DestFav/DestHist/DestWin` Popup+`AddHandler`+三控件+handler
- [x] 用 `QuickPathPicker` 替换解压目标输入区，`Path` bind `DestinationPath`，`BrowseAction= (owner,cur) => ShowExtractFolderAsync(owner,_entries,cur)`
- [x] lsp + build
- [x] commit `refactor(extract): use QuickPathPicker for destination path`（并入 cd6778f）
