...
| **P1** | 路径清单统一：预览=实际绝对一致 | [path-manifest-unification.md](.sisyphus/plans/path-manifest-unification.md) | 🟡中 | ~4.5h | **A/B 数据集方案（2026-08-07 重写）**：A=过滤前路径数据集，B=过滤后压缩计划（`CompressPlanItem(SourcePath, OutputArchivePath, IncludedFiles)`），**过滤只算一次**——预览构建时从树派生 B 并缓存，执行只读 B 不重算（按钮门禁 `IsBuildPending` 保证 B 不过期；CLI 无 B → planner 同公式重算）；预览 A/B 切换复用现成 `ShowFilteredGhosts`；输出包路径唯一实现 `CompressPathPlanner`（Core/Utils）：修「Separate 目录名含点被剥后缀」；引擎加文件白名单（`ArchiveOptions.FileWhitelist` + FileScanner 透传 + SevenZip 过滤激活改走展开路径）：修「压缩过滤 10 目录展开成 1000 文件 → Separate 生成 1000 个包」；解压侧 `FilteredEntryKeys` 即天然 B，仅修「解压过滤无效」（ExtractFlow 空匹配降级全量 + ZipEngine key 未归一化两处）；CompressFlow 的 KeepOriginalExtension 改读 VM；包内条目路径=同公式保证（Phase 2 再闭环） |
