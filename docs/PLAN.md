...
| **P2** | 压缩预览渐进式加载 | [compress-preview-progressive-loading.md](.omo/plans/未开始/compress-preview-progressive-loading.md) | 🟡中 | 4-6h | ✅ 已完成（2026-09-10）：两阶段加载（浅层先行→全量重建逐源替换→装配）、SourceSubtree 按源缓存（FilterSignature 签名失效）、ResultTreeView 滚动位置保持、占位节点 `IsLoadingPlaceholder` + `DisplayLabel`/`IsTruncatedNode`/`DirectoryInfoText`/`HasLoadingPlaceholderChild`/`ShallowClone`；`BuildDirectoryNode` 深度边界重构（`depth>=maxDepth` 时子目录挂占位、文件仍枚举）；`IsEmptyDirectory` 含占位视为非空；9 个单元测试 |
...
| **P2** | ~~结果预览冲突检测双模式~~ | [extract-preview-conflict-detection-optimization.md](.omo/plans/未开始/extract-preview-conflict-detection-optimization.md) | ✅已完 | 2-3h | ✅ 已实施：①②③ 短路优化 + 两阶段冲突检测（depth 2 快速上屏 + 全量后台补全）+ PreviewTreeInvalidated 事件 |
