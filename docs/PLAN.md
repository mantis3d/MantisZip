...
| **P1** | 压缩/解压性能优化（并行化 + 缓冲区） | [compression-performance-optimization.md](.omo/plans/未开始/compression-performance-optimization.md) | 🟡中 | 5-8h | 🟢 大部分完成（2026-09-16 核实，4/5 阶段）：① 缓冲区 256KB→4MB（ZipEngine/TarGzEngine/ZipBinaryRewriter）；② ZIP 并行解压（自研多实例并行 + Round-Robin 批次复用 + 进度锁竞争修复，`SupportsParallelExtract` + `ParallelExtractDegree` 设置 1-16）；③ **7z 多线程压缩（`mt=on`）实测 4.63x**（100×1MB，8核：38.7s→8.4s），含压缩对话框 + 设置窗口开关；④ 单测 Core 380 + Avalonia 96 全通过。**剩余**：ZIP 分组并行压缩（Task 7，需自研中央目录合并器，暂缓）+ 压力测试（可选） |
...
| **P2** | APNG 动画预览支持 | [apng-preview-support.md](.omo/plans/已完成/apng-preview-support.md) | 🟢低 | 2-3h | ✅ 已完成（2026-09-26）：复用 `AnimatedImage` 管线 + ImageSharp 解码器（`SixLabors.ImageSharp`），修复 Rational(uint) 类型转换；SKCodec 原生 `FrameCount=0` 需 ImageSharp 兜底，5帧 APNG 正常播放；测试 509/509 通过 |
