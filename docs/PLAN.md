...
| **P1** | 压缩/解压性能优化（并行化 + 缓冲区） | [compression-performance-optimization.md](.omo/plans/未开始/compression-performance-optimization.md) | 🟡中 | 5-8h | 📋 2026-09-14 立项：基准测试验证 100×1MB ZIP 解压串行 0.87s → 并行 0.14s（6.32x 提升），缓冲区 256KB→4MB 再提 1.7x；方案：SharpCompress 多实例并行（单实例线程不安全）+ CopyBufferSize 优化 + ParallelExtractDegree 设置 |
...
| **P2** | 字体预览连字效果开关 | [font-preview-ligature.md](.omo/plans/已完成/font-preview-ligature.md) | 🟡中 | 3-4h | ✅ 已完成（2026-07-05，v0.4.4）：HarfBuzzSharp shaping + `CheckFontSupportsLigature` 连字检测（liga feature 开/关 glyph 对比）+ `IsLigatureEnabled`/`ToggleLigature` 命令 + `CanLigatureToggle` 不支持时灰禁用 + 工具栏按钮 + `AppSettings.FontPreviewEnableLigature` 持久化 |
| **P2** | 压缩预览渐进式加载 | [compress-preview-progressive-loading.md](.omo/plans/已完成/compress-preview-progressive-loading.md) | 🟡中 | 4-6h | ✅ 已完成（2026-09-10）：两阶段加载（浅层先行→全量重建逐源替换→装配）、SourceSubtree 按源缓存（FilterSignature 签名失效）、ResultTreeView 滚动位置保持、占位节点 `IsLoadingPlaceholder` + `DisplayLabel`/`IsTruncatedNode`/`DirectoryInfoText`/`HasLoadingPlaceholderChild`/`ShallowClone`；`BuildDirectoryNode` 深度边界重构（`depth>=maxDepth` 时子目录挂占位、文件仍枚举）；`IsEmptyDirectory` 含占位视为非空；9 个单元测试 |
...
| **P2** | APNG 动画预览支持 | [apng-preview-support.md](.omo/plans/未开始/apng-preview-support.md) | 🟢低 | 2-3h | 复用 `AnimatedImage` 管线（GIF/WebP 动画已统一），仅需魔数检测区分 PNG/APNG（扫描 `acTL` chunk）+ 枚举/映射/显示名；SKCodec 原生支持 `FrameCount>1` 自动分流动画管线 |
