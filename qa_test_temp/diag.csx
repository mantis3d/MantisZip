...
var letters = new[] { "a", "m", "e", "x", "z", "." };
foreach (var ch in letters)
{
    // Substring mode
    var rs = ArchiveFilter.ApplyFilters(items, new SearchFilters { Text = ch, MatchMode = FilterMatchMode.Substring });
    // Wildcard mode
    var rw = ArchiveFilter.ApplyFilters(items, new SearchFilters { Text = ch, MatchMode = FilterMatchMode.Wildcard });
    Console.WriteLine($"Char '{ch}': Substring={rs.Count}/{items.Count}  Wildcard={rw.Count}/{items.Count}");
}
