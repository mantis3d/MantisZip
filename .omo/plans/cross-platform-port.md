...
> **状态**: 🔍 调研 | **任务**: [⬜⬜⬜⬜⬜⬜] (0/6)
> **创建日期**: 2026-06-11 | **更新日期**: 2026-06-13
...
> 5. **DPAPI** → AES-GCM + 平台密钥链（2 天替换 — **✅ 已完成**）
>
...
```
MantisZip.Core     → net9.0              ✅ 已跨平台（pure managed .NET，除 7z.dll；DPAPI 已替换为 AES-GCM）
...
```
PasswordManager.cs
  ├── Load()   → _protector.Unprotect()       ← IDataProtector 接口
  ├── Save()   → _protector.Protect()          ← IDataProtector 接口
  ├── CRUD 操作  → 全部在内存 JSON 对象上       ← 🟢 跨平台
  ├── 模式匹配   → 纯字符串/C# 代码              ← 🟢 跨平台
  └── 导入/导出  → 纯 JSON 序列化                ← 🟢 跨平台
```

**已实现**：AES-256-GCM（`System.Security.Cryptography.AesGcm`，跨平台）+ `IDataProtector` 接口：
- `AesGcmDataProtector`（当前默认）：密钥以文件形式存储在 `%APPDATA%/MantisZip/.masterkey`
- 未来可替换实现：macOS Keychain、Linux libsecret
- 旧 DPAPI 文件首次加载时自动迁移（备份为 `.dpapi-backup`）
...
| # | 阻塞项 | 工作量 | 说明 |
|---|--------|-------|------|
| 6 | **DPAPI → AES-GCM** ✅ | 2 天 | 深度浅，封装接口即可 — **已实现** `IDataProtector` + `AesGcmDataProtector` |
...
| # | 项 | 说明 |
|---|----|------|
| 11 | **Core 层（除 7z）** | 全部纯托管，无需改动；DPAPI 依赖已替换为 AES-GCM |
...
- **DPAPI → AES-GCM** ✅ — `IDataProtector` 接口 + `AesGcmDataProtector` 实现，已含旧文件自动迁移
