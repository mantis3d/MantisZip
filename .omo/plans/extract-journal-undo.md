...
- [ ] **1. Core: `ExtractJournal` 组件** — 日志记录 + 文件清单存储
- [ ] **2. Core: `Rollback` 回滚方法** — 倒序删除文件 + 清理空目录
- [ ] **3. UI: 提取历史面板** — 历史记录浏览 + 一键回滚按钮
- [ ] **4. 集成：提取流程钩子** — 每次解压自动记录日志
...
- [ ] `ExtractJournal.Record()` 在每次解压后自动记录文件清单
- [ ] 日志以 JSON 格式保存到 `%LOCALAPPDATA%\MantisZip\journal\`
- [ ] `ExtractJournal.Rollback()` 倒序删除 + 清理空目录
- [ ] UI 提取历史面板（浏览 + 回滚按钮）
- [ ] 回滚时路径不存在则跳过（不崩溃）
- [ ] `CleanupAsync` 定期清理旧日志
- [ ] SHA256 可选开关
- [ ] `dotnet build` 通过
