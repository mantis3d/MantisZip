...
- [x] **1.1** 创建 `src/MantisZip.UI.Avalonia/` 目录结构
- [x] **1.2** 编写 `MantisZip.UI.Avalonia.csproj`：
...
- [x] **1.3** 编写 `App.axaml` + `App.axaml.cs`：
  - `Application` 定义，引用 `MainWindow`
  - `OnFrameworkInitializationCompleted` 中 `new MainWindow{...}.Show()`
  - DataTemplates 注册 ViewModel → View 映射
- [x] **1.4** 编写 `Views/MainWindow.axaml` + `.cs`：
  - 最小骨架：标题栏 + 菜单栏 + 分割面板区域
  - 代码后置：只有 `InitializeComponent()` + 构造函数注入 VM
- [x] **1.5** 编写 `ViewModels/MainWindowViewModel.cs`：
  - 继承 `ObservableObject`
  - 空属性占位：`Title`、`ArchivePath`
- [x] **1.6** 验证：`dotnet build src\MantisZip.UI.Avalonia` 编译通过
- [x] **1.7** 验证：`dotnet run --project src\MantisZip.UI.Avalonia` 窗口启动
- [x] **1.8** **Commit**: `feat(avalonia): scaffold project skeleton with MainWindow`

### Task 2：文件列表模型 + 服务层

> 封装 Core 的 `ArchiveEngineFactory.ListEntriesAsync`，建立 MVVM 数据流。

- [x] **2.1** 编写 `Models/ArchiveItemModel.cs`：
  - 包裹 `Core.ArchiveItem`，添加 UI 展示属性：
    ...
    - `LastModifiedDisplay`
  - 继承 `ObservableObject`
- [x] **2.2** 编写 `Services/ArchiveService.cs`：
...
- [x] **2.3** 编写 `Converters/FileSizeConverter.cs`：
  - `IValueConverter`：`long` → `"1.23 MB"`（复用 Core 逻辑或独立实现）
- [x] **2.4** **Commit**: `feat(avalonia): add ArchiveService and ArchiveItemModel`
...
- [x] **3.3** `MainWindow.axaml` 布局：
  ...
  - 状态栏：左下角文件数 / 选中文件信息
- [x] **3.4** 编写 `Converters/BoolToVisibilityConverter.cs`：
  - 控制无文件时的提示文本显示/隐藏
- [x] **3.5** 验证：打开一个 ZIP，文件列表正常显示，列可排序
- [x] **3.6** **Commit**: `feat(avalonia): file list DataGrid with Open archive command`

### Task 4：文本预览

> 最简单的预览：检测编码 → 显示纯文本。

- [x] **4.1** `PreviewViewModel.cs` 编写：
  ...
  - 方法：`ClearPreview()`
- [x] **4.2** `MainWindowViewModel` 连接预览：
  - `SelectedEntry` 变化时触发预览
  - 调用 `ArchiveService` 临时提取文件到 `%TEMP%`
  - 调用 `PreviewViewModel.ShowTextPreview()`
- [x] **4.3** 更新 `MainWindow.axaml` 预览区域：
  - 根据 `PreviewContentType` 切换显示（`ContentControl` + `DataTemplate`）
  - 文本预览：`ScrollViewer` + `TextBox`（只读，等宽字体）
  - UTF-8 / GBK 测试文件显示正确
- [x] **4.4** 验证：打开含 .txt/.log/.csv（作为文本）查看预览
- [x] **4.5** **Commit**: `feat(avalonia): text preview with encoding detection`

### Task 5：CSV 预览

> CSV 以表格方式展示。

- [x] **5.1** `PreviewViewModel` 添加 CSV 支持：
  - 属性：`DataTable? CsvData`
  - 方法：`ShowCsvPreview(string filePath)`
    - 简单 CSV 解析（逗号分隔，首行表头）
    - 限制 100 行 × 100 列（与 WPF 版一致）
- [x] **5.2** 编写 `Controls/CsvDataGrid.axaml` + `.cs`：
  - 封装 `DataGrid` + 自动生成列
  - 无数据时的空状态文本
- [x] **5.3** `MainWindow.axaml` 添加 CSV 的 `DataTemplate`
- [x] **5.4** 验证：打开含 .csv 的压缩包，选中后显示表格
- [x] **5.5** **Commit**: `feat(avalonia): CSV preview with DataGrid`

### Task 6：PE 预览（exe/dll）

> 显示 PE 文件元数据：产品名、公司、版本、架构等。

- [x] **6.1** `PreviewViewModel` 添加 PE 支持：
  - 属性：`IReadOnlyList<(string,string)>? MetadataItems`
  - 方法：`ShowPePreview(string filePath)`
    - 调用 `PeParser.Parse(filePath)`（Core 已有）
    - 映射返回的字典到键值对列表
- [x] **6.2** `MainWindow.axaml` 添加 PE 预览 `DataTemplate`：
  - `ItemsControl` 显示键值对
  - 格式：分两栏（key 灰色，value 正常色）
- [x] **6.3** 验证：打开含 .exe/.dll 的压缩包，选中后显示元数据
- [x] **6.4** **Commit**: `feat(avalonia): PE metadata preview`
...
- [x] **7.2** 确认 `src/MantisZip.Core/` 未被修改
...
- [x] `dotnet build src\MantisZip.UI.Avalonia` ✅
- [x] `dotnet run --project src\MantisZip.UI.Avalonia` 启动空窗口 ✅
- [x] 打开一个 ZIP 文件，文件列表正常显示 ✅
- [x] 选中 .txt 文件，文本预览正常显示 ✅
- [x] 选中 .csv 文件，表格预览正常显示 ✅
- [x] 选中 .exe/.dll 文件，PE 元数据显示正常 ✅
- [x] WPF 项目 `src/MantisZip.UI/` 未被修改 ✅
- [x] Core 项目 `src/MantisZip.Core/` 未被修改 ✅
- [x] `git merge master` 无冲突 ✅
