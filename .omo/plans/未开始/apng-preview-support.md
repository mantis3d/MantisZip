...
- [ ] **1. Core 枚举扩展** — `FileFormat.Apng`、`FileFormatHelper.GetDisplayName`、"APNG 动画" 显示名
- [ ] **2. 魔数检测** — `FileFormatDetector.Detect` 扫描 PNG 后的 `acTL` chunk 识别 APNG
- [ ] **3. 预览映射** — `MapFileFormatToPreviewType` + `ClassifyPreview` 映射到 `AnimatedImage`
- [ ] **4. 验收测试** — 单元测试覆盖魔数检测、预览分类、播放功能
...
| 步骤 | 文件 | 变更 |
|------|------|------|
| 1 | `src/MantisZip.Core/Utils/FileFormatInfo.cs` | 新增 `Apng` 枚举 |
| 2 | `src/MantisZip.Core/Utils/FileFormatDetector.cs` | 新增 `ScanForActlChunk` + PNG 分支调用 |
| 3 | `src/MantisZip.Core/Utils/FileFormatHelper.cs` | 新增显示名 "APNG 动画" |
| 4 | `src/MantisZip.UI.Avalonia/Services/PreviewService.cs` | `MapFileFormatToPreviewType` + `ClassifyPreview` 扩展名 |
| 5 | `tests/MantisZip.Tests/Utils/FileFormatDetectorTests.cs` | 新增 APNG 魔数检测测试用例 |
...
- [ ] 单元测试覆盖 APNG 魔数检测（正例/反例）
- [ ] 手动验证：APNG 文件预览播放正常、工具栏显示正确、信息面板显示正确
- [ ] `dotnet build` 通过，无回归

### Final Checklist

- [ ] APNG 魔数检测正确（含 `acTL` chunk 的 PNG → Apng，不含 → Png）
- [ ] 预览类型正确映射到 `AnimatedImage`
- [ ] 动画播放功能完整（播放/暂停/上帧/下帧/帧输入/循环）
- [ ] 工具栏按钮：缩放、透明背景、动画控制均显示
- [ ] 信息面板显示 "APNG 动画" + 尺寸 + 帧数
