...
- [ ] **1. `ArchiveItem.SizeRatio` 属性** — 计算属性 + 列表加载/过滤时赋值
- [ ] **2. `RatioToWidthConverter`** — 值转换器或替代方案
- [ ] **3. XAML 模板替换** — `DataGridTextColumn` 替换为 `DataGridTemplateColumn`
...
- [ ] `ArchiveItem.SizeRatio` 属性在列表加载/过滤时正确计算
- [ ] 大小列背景进度条随文件大小比例显示
- [ ] 切换目录/过滤时进度条重新计算
- [ ] 暗色主题下颜色正常
- [ ] `dotnet build` 通过
