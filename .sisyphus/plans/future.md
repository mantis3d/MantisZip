...
**目标**：恢复并完善工具栏筛选功能（当前无实际效果）。

- 搜索框：在文件列表上方加 TextBox，实时过滤当前目录下文件名包含关键词的条目
- 子目录切换：恢复"显示子目录"复选框功能，选中时 `FilterFiles` 不过滤深度，显示所有嵌套条目
- 与现有目录树导航、双击进入子目录等交互协同

**优先级**：中（当前目录树 + 双击导航基本可用）

## 3. 未知扩展名智能文本检测（解压前 512B）
