...
- [ ] **2. Core: `CompressionCoefficients` 经验系数表** — 扩展名 → 类型 → 压缩率
- [ ] **3. UI: `CompressSettingsWindow` 预估面板** — XAML 布局 + 数据绑定
- [ ] **4. UI: 预估交互逻辑** — 自动检测 + 刷新按钮 + 防抖
- [ ] **5. Test: 单元测试** — `CompressionEstimatorTests`
...
**魔数检测实现**：>64KB 大文件的魔数识别依赖 `preview-magic-detection.md` 中的检测基础设施。

| 场景 | 检测方案 | 详情 |
|------|---------|------|
| 主路径 | **Mime-Detective 库**（见预览计划） | 调用库检测 `byte[]` head → 映射到 `CompressionCoefficients` 分类（text/image_lossy/media/archive/binary） |
| 回退 | `ClassifyByExtension()` | 库检测失败或文件 < 64KB 时降级使用 |
| 依赖 | `ExtractHeadAsync` | 同预览计划共享——从压缩包或文件读取 head 字节 |

Mime-Detective 返回的格式结果按以下规则映射到压缩系数分类：

```
Mime-Detective 检测结果                       CompressionCoefficients 分类
─────────────────────────                     ────────────────────────
image/jpeg, image/webp  ──────────→           image_lossy (压不动, 压缩率 ~0.99)
image/png, image/bmp    ──────────→           image_lossless (可压, 压缩率 ~0.85)
image/gif               ──────────→           image_lossless
video/*, audio/*        ──────────→           media (压不动, 压缩率 ~1.00)
application/zip, application/x-7z-compressed  → archive (已压缩, 压缩率 ~1.00)
application/pdf         ──────────→           binary (可压, 压缩率 ~0.60)
text/plain, application/xml  ──────→          text (高压缩比, 压缩率 ~0.15)
```

> **引用**：魔数检测的完整方案（Mime-Detective 库 + 手动回退）见
> [`preview-magic-detection.md` → 方案选择：库 vs 手动魔数](preview-magic-detection.md)。
