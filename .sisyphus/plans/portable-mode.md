...
- [ ] **3. `App.OnStartup` — 跳过 Shell 注册** — 便携版不安装右键菜单（跳过 `InstallCom()` 的 CLSID 写入 + `SyncContextMenuToRegistry()`）
...
| 文件 | 改动量 | 说明 |
|------|--------|------|
| `AppSettings.cs` | ~20 行 | 静态构造 + `IsPortableMode` 属性 + `_settingsPath` 重定向 + `Save()` 跳过 `SyncContextMenuToRegistry()` |
| `PasswordManager.cs` | ~10 行 | `CustomDataDir` 注入 + `GetPasswordsPath()` |
| `App.xaml.cs` | ~5 行 | 启动时跳过 `--install-shell` 和 `InstallCom()` |
| `ShellIntegration.cs` | ~3 行 | `Install()` 中 `IsPortableMode` 时跳过 COM 和静态注册 |
...
- [ ] 便携模式下跳过 Shell 右键菜单注册（跳过 `InstallCom()` + `SyncContextMenuToRegistry()`）
...
- [ ] 便携版设置随 exe 位置移动
- [ ] 便携版密码库随 exe 位置移动
- [ ] 便携版不写注册表（Shell 菜单不安装、COM CLSID 不写入、设置同步绕过 `SyncContextMenuToRegistry()`）
- [ ] 便携版预览临时文件不写入系统 Temp
